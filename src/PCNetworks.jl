...
# Trains the PCNetwork in an unsupervised manner. Uses a discrete callback function to pause the ODE solver at the times in stops and update each layers' parameters.
function train!(m::PCNet, x::Tuple, tspan = (0.0f0, 100.0f0); solver = BS3(), iters = 1, stops = [tspan[1], (tspan[2] - tspan[1]) / 2, tspan[2]], decayLrEvery = 500, lrDecayRate = 0.85f0, show_every = 10, normalize_every = 10000, abstol = 0.01f0, reltol = 0.05f0, save_everystep = false)
    ...
    j = StatsBase.sample(1:length(x), 1)[1]
    xj = cu(x[j]) 
    yj = cu(y[j])
    ...
    for i in 1:iters
        ...
        #initialize the network
        values(NamedTuple(m.odemodule.u0))[1] .= xj
        for k in eachindex(m.odemodule.layers)
            m.odemodule.layers[k].is_supervised = false
            m.odemodule.layers[k].initializer!(values(NamedTuple(m.odemodule.u0))[k + 1], values(NamedTuple(m.odemodule.u0))[k])
        end
...
# Batch trains the PCNetwork in a supervised manner.
function train!(m::PCNet, x::Tuple, y::Tuple, tspan = (0.0f0, 100.0f0); solver = BS3(), iters = 1, stops = [tspan[1], (tspan[2] - tspan[1]) / 2, tspan[2]], switch_every = 1, decayLrEvery = 500, lrDecayRate = 0.85f0, show_every = 10, normalize_every = 10000, abstol = 0.01f0, reltol = 0.05f0, save_everystep = false)
    ...
    for i in 1:iters
        ...
        m.odemodule.layers[end].is_supervised = true
        m.odemodule.layers[end].initializer!(values(NamedTuple(m.odemodule.u0))[end], values(NamedTuple(m.odemodule.u0))[end - 1])
        m.odemodule.layers[end].labels .= yj
