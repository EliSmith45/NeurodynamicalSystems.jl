...
export PCLayer, change_nObs!, to_gpu!, to_cpu!, PCStaticInput, PCDynamicInput, PCDense, DenseInitializer, PCConv, ConvInitializer, PCModule, ModuleInitializer
...
function PCDense(in_dims, out_dims, name::Symbol, T = Float32; prop_zero = 0.5, σ = relu, tc = 0.1f0, α = 0.01f0)
    ...
    PCDense(out_dims, ps, σ, grads, ps2, receptiveFieldNorms, tc, α, is_supervised, name, T), initializer!
end
...
function PCConv(k, ch::Pair, in_dims, name::Symbol, T = Float32; prop_zero = 0.5, σ = relu, stride=1, padding=0, dilation=1, groups=1, tc = 1.0f0, α = 0.01f0)
    ...
    PCConv(size(states), ps, cdims, σ, grads, ps2, receptiveFieldNorms, tc, α, is_supervised, name, T), initializer!
end
...
function (m::PCConv)(errors_lm1, u_l)
    ...
    m.ps[k] .+= ((m.α * size(u_l, ndims(u_l) - 1)) / prod(size(errors_lm1))) .* ∇conv_filter!(m.grads, u_l, errors_lm1, m.cdims)
    m.ps .= relu(m.ps)
...
# Makes the ConvInitializer callable for training. Sets the first scale level of u0 equal to the input x, then computes the initial values of each scale level
# using the learnable feedforward network parameterized by ps.
...
Construct a dense predictive coding layer.
"""
function PCConv2Dense(in_dims, out_dims, name::Symbol, T = Float32; prop_zero = 0.5, σ = relu, tc = 0.1f0, α = 0.01f0)
    ...
    #grads_initializer = deepcopy(ps_initializer)
    initializer! = Conv2DenseInitializer(ps_initializer, inputsizeFlat, σ, grads, α)
    
    PCConv2Dense(out_dims, ps, σ, predictedFlat, errorsFlat, grads, ps2, receptiveFieldNorms, tc, α, is_supervised, name, T), initializer!
end
...
function (m::PCConv2Dense)(du_l, u_l, predictions_lm1, errors_lm1, errors_l, labels)
    ...
    if m.is_supervised
        ...
        u_l .= m.σ(u_l) 
        du_l .= mul!(du_l, transpose(m.ps), m.reshape(errors_lm1, size(m.predictedFlat))) .- errors_l
        predictions_lm1 .= reshape(mul!(m.predictedFlat, m.ps, u_l), size(predictions_lm1))
...
function (m::PCConv2Dense)(errors_lm1, u_l)
    ...
    m.ps .+= (m.α / size(errors_lm1, 2)) .* mul!(m.grads, reshape(errors_lm1, size(m.predictedFlat)), transpose(u_l))
    m.ps .= relu(m.ps)
...
function (m::Conv2DenseInitializer)(initerror_l, u0_lm1, uT_l)
    ...
    m.ps .+=  (m.α / size(initerror_l, 2)) .* mul!(m.grads, reshape(u0_lm1, m.inputsizeFlat), transpose(initerror_l))
...
Constructor for PCStaticInput ayers.
"""
function PCStaticInput(in_dims::Tuple, name::Symbol, T = Float32)
...
"""
function PCModule(inputlayer, hiddenlayers, initializers)

    nObs = size(inputlayer.states, ndims(inputlayer.states))
    ...
    PCModule(nObs, inputlayer, hiddenlayers, initializers, u0, predictions, errors, labels), initializer!
end
