...
## Intuition 
To see the intution here, it is important to first understand the role that depth plays in deep neural networks (DNNs). Most often, the given explanation is that they can learn and express hierarchical patterns in structured data. While true, depth plays another role, which is arguably far more important: they can nonlinearly "sharpen" a blurred input to create better sparse codes. 
